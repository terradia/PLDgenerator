...
def main():
    ...
    @return:
    """
    #UserStorie("1.1 Modification du BMC", "CBO (Chief Business Officer)", "BMC qui définit bien le projet", "Le BMC permet de dresser l’état des lieux du modèle économique d’une entreprise. Il est donc important qu’il soit précis", "-	Remplir le BMC\n-	Effectuer les modifications sur les parties principales\n-	Définir précisément les cibles avec les critères de segmentation\n-	Modification sur les parties Channels et Customer Relationship\n-	Faire une petite étude de la consommation moyenne pour définir les cibles\n-	Faire des recherches sur les catégories socio-professionnelles", "1 Jour(s) / Homme")
